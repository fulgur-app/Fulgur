...
#[cfg(all(feature = "gpui-test-support", target_os = "macos"))]
use gpui_kit::{Entity, TestAppContext, WindowId};

/// Invoke `process_pending_files_from_macos` on the window with the given id.
///
/// ### Arguments
/// - `cx`: The test application context
/// - `window_id`: The id of the window to run the callback on
/// - `fulgur`: The Fulgur entity owned by that window
#[cfg(all(feature = "gpui-test-support", target_os = "macos"))]
