...
impl Render for StatusBar {
    ...
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let active_tab = fulgur.active_tab(cx);
        ...
        let is_large_file = active_editor_tab.is_some_and(|tab| tab.large_file);
        let current_language =
            active_editor_tab.map_or(SupportedLanguage::Plain, |tab| tab.language);
        let is_csv = current_language == SupportedLanguage::Csv;
        ...
        let log_path = active_editor_tab.and_then(|tab| tab.file_path().cloned());
