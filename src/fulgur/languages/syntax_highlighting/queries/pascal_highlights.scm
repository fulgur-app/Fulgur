...
; -- Property / field access via dot notation (e.g. Greeter.Counter)
; Covers reads of properties, fields, and no-argument method results that are
; not wrapped in an exprCall (those are handled above).
(exprDot rhs: (identifier) @function)
...
; NOTE: (identifier) @identifier is intentionally omitted.
; This catch-all would override all heuristic highlights above via last-match-wins
; rendering (merge_highlight_style), since @identifier maps to no visible style.
; It was designed to work alongside a locals query for scope-based highlighting,
; but locals are disabled (we pass "" to avoid the @local.definition override bug).
; Without locals, the catch-all serves no purpose and breaks constant/function detection.
