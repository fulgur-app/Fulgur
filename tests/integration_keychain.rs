//! Integration tests for Keychain storage and retrieval
//!
//! These tests verify keychain operations using a test-specific service name
//! to avoid interfering with production data. All test entries are cleaned up
//! after each test.
//!
//! **All tests in this file are `#[ignore]`d by default** and must be run
//! explicitly with `-- --include-ignored`. This prevents linking
//! `Security.framework` into the normal test run, which can trigger a macOS
//! keychain access prompt even when the tests themselves are skipped.
//!
//! Run these locally with:
//! ```sh
//! cargo test --test integration_keychain -- --include-ignored
//! ```
//!
//! ## Platform Support
...
fn test_full_integration_keychain_and_encryption() {
    use fulgur::fulgur::utils::crypto_helper::{decrypt_bytes, encrypt_bytes};
