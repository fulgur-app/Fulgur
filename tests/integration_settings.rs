...
fn create_custom_settings() -> Settings {
    ...
    settings.app_settings.scrollbar_show = Some(gpui_kit::component::scroll::ScrollbarMode::Always);
...
fn test_settings_optional_fields_some() {
    ...
    let settings_path = temp_settings_path(&temp_dir);
    let mut original = Settings::new();
    original.app_settings.scrollbar_show = Some(gpui_kit::component::scroll::ScrollbarMode::Hover);
    let mut profile = ServerProfile::new("Profile");
    ...
    assert_eq!(
        ...
        Some(gpui_kit::component::scroll::ScrollbarMode::Hover)
    );
    let profile = loaded
        .app_settings
        .synchronization_settings
